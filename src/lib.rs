...
//! using the [simdeez](https://github.com/jackmott/simdeez) crate. It also provides an implementation which uses [OpenCV](https://opencv.org/)'s (A C++ library)
//! matchTemplate function using the [opencv-rust](https://github.com/twistedfall/opencv-rust) crate through an optional off-by-default
...
pub enum Backend {
  ...
  /// This should detect CPU features at runtime and use the best possible rust SIMD implementation
  /// of SQDIFF_NORMED (square difference).
  ///
...
/// The default value used in [SubImageFinderState::new] and [Default::default]
pub const NONOPENCV_DEFAULT_THRESHOLD: f32 = 0.1;

impl SubImageFinderState {
  /// Create a SubImageFinderState
  ///
  /// This uses the Scalar backend by default, unless the "simdeez-default-new" e is enabled (It is
  ...
  /// Finds positions where the subimage is found within the search image. These positions represent
  /// the top-right corner of the subimage.
  ///
  ...
  /// results.
  ///
  /// This is done using internal buffers. If you reuse a [SubImageFinderState] for multiple images
  /// of the same size, it should only need to allocate once.
  ///
  ...
  fn find_subimage_positions_with_backend_impl(
    ...
    match *backend {
      ...
      Backend::RuntimeDetectedSimd {
        ...
        let width_multiplier = if to_grayscale {
          1
        } else {
          subimage_channel_count as usize
        };
      ...
      Backend::OpenCV { threshold } => {
        let subimage_ptr: *mut std::ffi::c_void = if to_grayscale && subimage_channel_count != 1 {
          self.u8buf_subimage.clear();
          self.u8buf_subimage.extend(
            subimage
              .chunks_exact(subimage_channel_count as usize)
        ...
        let search_ptr: *mut std::ffi::c_void = if to_grayscale && search_image_channel_count != 1 {
          self.u8buf_search_image.clear();
          self.u8buf_search_image.extend(
            search_image
              .chunks_exact(search_image_channel_count as usize)
...
mod tests {
  ...
  //TODO: Tests and benchmarks in this crate (I've been doing them in a separate binary crate that
  // used this as a path dependency)
}
